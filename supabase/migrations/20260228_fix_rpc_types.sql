-- ============================================
-- FIX: OPERATOR PRECEDENCE IN RPC
-- ============================================
-- The previous version of the RPC failed with "operator does not exist: text ->> unknown" 
-- because `v_products_summary || v_item->>'name'` grouped left-to-right.
-- This rewrites the RPC with explicit parentheses around JSON extractions.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Fix the precedence error here by wrapping v_item->>'name' in parentheses
